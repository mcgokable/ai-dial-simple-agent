...
class DialClient:
    ...
    def __init__(
        ...
        self._tool_schemas = [tool.schema for tool in tools] if tools else []
        # 6. Optional: print endpoint and tools schemas
        print("Endpoint:", self.__endpoint)
        print("Tools schemas:", self._tool_schemas)
    ...
    def get_completion(self, messages: list[Message], print_request: bool = True) -> Message:
        ...
        if response.status_code == 200:
            ...
            if choices:
                ...
                return ai_response
        raise RuntimeError(f"Error getting completion: {response.status_code}: {response.text}")
    ...
    def _process_tool_calls(self, tool_calls: list[dict[str, Any]]) -> list[Message]:
        ...
        for tool_call in tool_calls:
            ...
            tool_messages.append(Message(role=Role.TOOL, name=function_name, tool_call_id=tool_call_id, content=tool_execution_result))
    ...
    def _call_tool(self, function_name: str, arguments: dict[str, Any]) -> str:
        ...
        tool = self._tools.get(function_name, None)
        if tool:
